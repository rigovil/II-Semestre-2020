...
//   if(send(this->idSocket, text, len, 0) < 0){
...
int Socket::Bind( int port ) {
  ...
  addr.sin_addr.s_addr = htonl(INADDR_ANY); //vincula a todas las redes locales 
  addr.sin_port = htons(port);

  int len = sizeof(addr);

  
  if (bind(this->idSocket, (sockaddr*) &addr, len)){
...
int Socket::Shutdown( int mode ) {
  if(shutdown(this->idSocket, mode) < 0){
