...
std::string responseHTTP(char * buf) {
  ...
  http_response.append(status_line);    /* este es la versión del http y el código de estado */
                                        /* aquí iría el resto del header */
  http_response.append(CRLF);           /* este es solo un cambio de línea que se tiene que hacer según el protocolo */
  http_response.append(message_body);   /* este es el contenido HTML */

  /**
   * Faltan los otros campos del header y concatenarlos en la variable http_response.
   */
...
void * receive(void * arg) {
  ...
  s2 = (Socket *) arg;
  buffer_received = (char *) malloc(BYTES_READ);
  
  s2->Read(buffer_received, BYTES_READ);

  http_response = responseHTTP(buffer_received);
  buffer_sent = (char *) malloc(http_response.length()+1);
  strcpy(buffer_sent, http_response.c_str());

  s2->Write(buffer_sent);

  std::cout << "\nRespuesta enviada al cliente:\n";
  std::cout << buffer_sent << NEW_LINE << NEW_LINE;
