...
std::string findRegion(std::ifstream &file) {
  ...
  char * block;
  std::string line, data, answer, old_answer, covid_data;
  ...
  while(getline(file, line)) {
    ...
    if(!first) {
      ...
      while(getline(stream, data, DELIMITER) && !found) {
        ...
        if(!i && (!answer.compare(translator(data)) || needleInHaystack(translator(data), old_answer))) {
  ...
  if((!match) || !old_answer.compare(translator(INFO1))) {
...
void covidData(int region, std::ifstream &file) {
	...
	if(region == CR) {
		...
		std::vector<int> otros_acumulador(ARRAY_SIZE);
		std::vector<int> CR_Statistics(ARRAY_SIZE);
        ...
        std::string region = line.substr(L1, L2);
				...
				if(INFO1 != region) {
					while(getline(stream, data, DELIMITER)) {
						...
						if(i) {
							CR_Statistics[i-1] += std::stoi(data);
						}
					...
					if(first_o) { 
						while(getline(stream, data, DELIMITER)) {
							...
							if(i) {
                CR_Statistics[i-1] += std::stoi(data);
								otros_acumulador[i-1] += std::stoi(data);
							}
    ...
    for(int i = ZERO; i < ARRAY_SIZE; ++i) {
      ...
      table.add(std::to_string(otros_acumulador[i]));
    ...
    for(int i = ZERO; i < ARRAY_SIZE; ++i) {
      ...
      table.add(std::to_string(CR_Statistics[i]));
    ...
    std::string data = findRegion(file), region_data;
