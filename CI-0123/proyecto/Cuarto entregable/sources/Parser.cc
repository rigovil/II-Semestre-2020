 ...
 * @return array que contiene si el cliente es uan terminal, la región solicitada y un indicador si lo solicitado es un país o cantón.
 */

std::vector<std::string> Parser :: readRegion(std::string user_region) {
    
    bool match_table, match_canton, match_country;
    std::string region;
    std::vector<std::string> region_info;
    std::smatch matches_table, matches_canton, matches_country;
    std::regex reg_exp_table (REG_EXP_T);
    std::regex reg_exp_canton (REG_EXP_R);
    std::regex reg_exp_country (REG_EXP_C);

    match_table     = std::regex_search(user_region, matches_table, reg_exp_table);
    match_canton    = std::regex_search(user_region, matches_canton, reg_exp_canton);
    match_country   = std::regex_search(user_region, matches_country, reg_exp_country);

    if(match_canton) {
        region = matches_canton.str(MATCH);
        region.erase(region.begin(), region.begin()+8);
        
        if(match_table) {
            region_info.push_back(TABLE);
        }
        else {
            region_info.push_back(NO_TABLE);
        }

        region_info.push_back(CANTON);
        region_info.push_back(region);
    }
    else if(match_country) {
        region = matches_country.str(MATCH);
        region.erase(region.begin(), region.begin()+9);
...
std::string Parser :: countryJSON(std::string file_name, bool exists) {
    ...
    //data = this->readCountry(response); // ver esta parte
