...
int main(int argc, char ** argv) {
     ...
     * 
     */
    std::ifstream file("docPrueba.csv");
 ...
 */
std::string manejaExpresionesDeConsulta(std::string aBuscar){
...
/**
 * buscar en archivo, pero necesita manejaExpresionesDeConsulta(std::string aBuscar), además si es igual guarda en qué linea lo encontró
 */
...
void serverReader(std::string server_address, int workers) {
	...
	file.open(PARSED_FILE);      /* Nombre de prueba para el archivo que se crea en el disco */
	file << "Lugar" << DELIMITER << "Activos" << DELIMITER  << "Recuperados" << DELIMITER  << "Fallecidos" << DELIMITER  << "Recuperados" << std::endl;
