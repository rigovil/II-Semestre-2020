 ...
 *
 * Utiliza: fork (procesos), Semaforos y Memoria Compartida
 *
...
double SumarUnoControlado( int semid, long * suma ) {
   ...
   long miSuma = 0;	// Utilizamos esta variable local para solo bloquear la variable compartida una vez
			// La otra manera es bloquear antes de for y desbloquear despues del for, sin variable local

   for ( i = 0; i < 1000; i++ ) {
      miSuma++;		// Realiza algun trabajo, en este caso sumar 1 es muy poco, se puede utilizar usleep( 1 );
   }			// para alargar un poco la duracion de la operacion
